      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 lg:grid-cols-12">
        <div className="flex min-h-0 flex-col gap-4 lg:col-span-8">
          ...
          <div className="flex h-44 shrink-0 flex-col rounded-xl border border-[#1E293B] bg-[#0B0F19] p-3 lg:h-48">
            <div className="mb-1 flex shrink-0 items-center justify-between"><span className="text-[10px] font-bold uppercase text-slate-300">{snapshotConnected ? 'Live telemetry' : 'Replay telemetry'} · current trip</span><div className="flex gap-3 font-mono text-[9px] text-slate-400"><span className="text-sky-400">— Speed</span><span className="text-indigo-400">— Risk Score</span></div></div>
        ...
        <div className="flex min-h-0 flex-col gap-4 lg:col-span-4">
          <div className="shrink-0 rounded-xl border border-[#1E293B] bg-[#0B0F19] p-4">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-center"><Metric label="Safe Driving Score" value={formatNumber(safetyScore)} /><Metric label="Risk Score" value={formatNumber(displaySnapshot?.risk_score)} /><Metric label="TTC" value={displaySnapshot?.predicted_ttc_sec === null ? '\u221e' : formatNumber(displaySnapshot?.predicted_ttc_sec, 2)} /></div>
            <p className="mt-3 text-[9px] text-slate-500">Live Safe Driving Score/Risk Score/TTC come from the AI Challenge 3 snapshot; if the backend is offline, this card falls back to the saved frame.</p>
          </div>
