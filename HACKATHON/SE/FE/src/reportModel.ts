...
export const buildCopilotInput = (models: VehicleReportModel[], reportMode: ReportMode) => ({
  ...
  fleet_summary: {
    tripCount: models.length,
    avgScore: models.length ? models.reduce((sum, model) => sum + model.score, 0) / models.length : 0,
    avgRisk: models.length ? models.reduce((sum, model) => sum + model.avgRisk, 0) / models.length : 0,
    maxRisk: models.length ? Math.max(...models.map((model) => model.maxRisk)) : 0,
    totalHighRiskFrames: models.reduce((sum, model) => sum + model.rawCriticalRiskFrames, 0),
    totalNearMissCount: models.reduce((sum, model) => sum + model.nearMissCount, 0),
    totalHarshBrakeCount: models.reduce((sum, model) => sum + model.harshBrakeCount, 0),
    totalFatigueEvents: models.reduce((sum, model) => sum + model.fatigueEvents, 0),
    avgDistractedPct: models.length ? models.reduce((sum, model) => sum + model.distractedPct, 0) / models.length : 0,
    totalEvents: models.reduce((sum, model) => sum + model.eventSummary.total, 0),
    dangerEvents: models.reduce((sum, model) => sum + model.eventSummary.danger, 0),
    warningEvents: models.reduce((sum, model) => sum + model.eventSummary.warning, 0),
    riskBuckets: {
      safe: models.filter((model) => model.riskLevel === 'SAFE').length,
      watch: models.filter((model) => model.riskLevel === 'WATCH').length,
      atRisk: models.filter((model) => model.riskLevel === 'AT_RISK').length,
      critical: models.filter((model) => model.riskLevel === 'CRITICAL').length,
    },
    highestRankedTrip: [...models].sort((a, b) => a.rank - b.rank)[0]?.tripId ?? 'N/A',
    highestRankedScore: [...models].sort((a, b) => a.rank - b.rank)[0]?.score ?? 0,
    reviewPriorityOrder: [...models].sort((a, b) => a.score - b.score).map((model) => model.tripId),
    maintenancePriority: {
      inspect: models.filter((model) => model.maintenance.priority === 'INSPECT').length,
      watch: models.filter((model) => model.maintenance.priority === 'WATCH').length,
      normal: models.filter((model) => model.maintenance.priority === 'NORMAL').length,
    },
  },
